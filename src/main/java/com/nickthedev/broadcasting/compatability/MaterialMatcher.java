...
/**
 * Adapted material transformer from @Hex_27
 *
 * @author NickTheDev
 * @author Hex_27
 * @since 3.0
 */
public enum MaterialMatcher {
    ...
    MaterialMatcher(int data, String... m){
        this.m = m;
        this.data = data;
    }

    private static HashMap<String, MaterialMatcher> cachedSearch = new HashMap<>();
    public static MaterialMatcher request(String name, byte data) {
        if(cachedSearch.containsKey(name.toUpperCase()+","+data)){
            return cachedSearch.get(name.toUpperCase()+","+data);
        }
        for(MaterialMatcher mat:MaterialMatcher.values()){
    ...
    public static MaterialMatcher fromString(String key){
        MaterialMatcher xmat = null;
        try{
            xmat = MaterialMatcher.valueOf(key);
            return xmat;
        }catch(IllegalArgumentException e){
            String[] split = key.split(":");
            if(split.length == 1){
                xmat = request(key,(byte) 0);
            }else{
                xmat = request(split[0],(byte) Integer.parseInt(split[1]));
            }
            return xmat;
        }
